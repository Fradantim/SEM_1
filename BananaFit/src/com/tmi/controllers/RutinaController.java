...
public class RutinaController {
	...
	public List<RutinaDTO> getRutinasPublicas(Map<String, Object> attributes, boolean inclusive){
		if(attributes==null) {
			attributes= new HashMap<>();
		}
		return rutinaDao.findRutinasPublicasByAttributes(attributes, inclusive).stream().map(Rutina::toDTO).collect(Collectors.toList());
	}
	
	public List<RutinaDTO> getRutinasCreadas(Usuario usuario){
		return usuario.getRutinasCreadas().stream().map(Rutina::toDTO).collect(Collectors.toList());
	}
	
	public List<RutinaDTO> getRutinasCreadas(Integer idUsuario) throws ObjetoInexistenteException{
		Usuario usuario = usuarioDao.getById(idUsuario);
		return getRutinasCreadas(usuario);
	}
	
	public List<RutinaDTO> getRutinasAsociadas(Usuario usuario) {
		return usuario.getRutinasAsociadas().stream().map(Rutina::toDTO).collect(Collectors.toList());
	}
	
	public List<RutinaDTO> getRutinasAsociadas(Integer idUsuario) throws ObjetoInexistenteException{
		Usuario usuario = usuarioDao.getById(idUsuario);
		return getRutinasAsociadas(usuario);
	}
	
	public RutinaDTO altaRutina(Integer idUserCreador, String nombre, String descripcion, int series, int duracionEjercicio, Boolean publica) throws ObjetoInexistenteException {
		Usuario user = usuarioDao.getById(idUserCreador);
		return altaRutina(user, nombre, descripcion, series, duracionEjercicio,publica);
	}
	
	public RutinaDTO altaRutina(Usuario userCreador, String nombre, String descripcion, int series, int duracionEjercicio, Boolean publica) {
